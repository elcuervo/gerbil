...
var Gerbil = function(description, tests, logger) {
  ...
  this.assert_equal = function(first, second) {
    if (!first || !second) throw new Error("attr1 = " + first + " (" + first.name+ ")" + "and attr2 = " + second + " (" + second.name + ")");
    if ( typeof(first) != typeof(second) ) throw new Error("Different type " + first.name + " vs " + second.name);
