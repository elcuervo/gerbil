...
var Gerbil = function(description, tests, logger){
  ...
  this.logger = typeof(logger) == 'Function' ? logger : this.pretty_console;
    ...
    switch(first.constructor){
      case Array:
        ...
        for (var i = 0; i < first.length; i++){
          if (first[i] != second[i]) throw new Error("Items not equal");
        }
        break;
      case String, Number:
        if (first != second) throw new Error("Not equal");
        break;
    }
  ...
  this.consume = function(){
    console.log("There are " + this.count + " tests to execute");
  ...
  this.summary = function(){
    ...
    this.logger.error(this.failures);
  };

  return this;
}

module.exports = function(description, tests, logger){
